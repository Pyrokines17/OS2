...
queue_t* queue_init(int max_count) {
	...
	if (err != EXIT_SUCCESS) {
		...
		if (err != EXIT_SUCCESS) {
			fprintf(stderr, "queue_init: pthread_spin_destroy() failed: %s\n", strerror(err));
		}
...
int queue_destroy(queue_t *q) {
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_destroy: pthread_spin_lock() failed: %s\n", strerror(stat));
		return stat;
	}
	...
	if (stat != EXIT_SUCCESS) {
		...
		statOt = pthread_spin_unlock(&q->spinlock);

		if (statOt != EXIT_SUCCESS) {
			fprintf(stderr, "queue_destroy: pthread_spin_unlock() id 1 failed: %s\n", strerror(statOt));
			return statOt;
		}
	...
	if (stat != EXIT_SUCCESS) {
		...
		if (statOt != EXIT_SUCCESS) {
			fprintf(stderr, "queue_destroy: pthread_spin_unlock() id 2 failed: %s\n", strerror(statOt));
			return statOt;
		}
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_destroy: pthread_spin_unlock() id 3 failed: %s\n", strerror(stat));
		return stat;
	}
...
int queue_add(queue_t *q, int val) {
	...
	if (q->count == q->max_count) {
		...
		if (stat != EXIT_SUCCESS) {
			fprintf(stderr, "queue_add: pthread_spin_unlock() id 1 failed: %s\n", strerror(stat));
			return UNLOCK_ERROR;
		}
	...
	if (new == NULL) {
		...
		if (stat != EXIT_SUCCESS) {
			fprintf(stderr, "queue_add: pthread_spin_unlock() id 2 failed: %s\n", strerror(stat));
			return UNLOCK_ERROR;
		}
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_add: pthread_spin_unlock() id 3 failed: %s\n", strerror(stat));
		return UNLOCK_ERROR;
	}
...
int queue_get(queue_t *q, int *val) {
	...
	*val = tmp->val;
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_get: pthread_spin_unlock() id 2 failed: %s\n", strerror(stat));
		return UNLOCK_ERROR;
	}
...
void queue_print_stats(queue_t *q) {
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_print_stats: pthread_spin_lock() failed: %s\n", strerror(stat));
		pthread_exit(NULL);
	}
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_print_stats: pthread_spin_unlock() failed: %s\n", strerror(stat));
		pthread_exit(NULL);
	}
}
