...
Storage* storage_init(int max_count) {
	...
	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_init: pthread_rwlock_init() failed: %s\n", strerror(stat));
		return NULL;
	}
...
void storage_destroy(Storage* s) {
	...
	for (int i = 0; i < len; ++i) {
		...
		if (stat != EXIT_SUCCESS) {
			fprintf(stderr, "queue_destroy: pthread_rwlock_destroy() failed: %s\n", strerror(stat));
			return;
		}
...
int storage_add(Storage* s, char* val) {
	...
	int stat = pthread_rwlock_init(&new->sync, NULL);

	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_add: pthread_rwlock_init() failed: %s\n", strerror(stat));
		return FALSE;
	}
...
int storage_get(Storage* s, char** val) {
	...
	int stat = pthread_rwlock_destroy(&tmp->sync);

	if (stat != EXIT_SUCCESS) {
		fprintf(stderr, "queue_get: pthread_rwlock_destroy() failed: %s\n", strerror(stat));
		return FALSE;
	}
